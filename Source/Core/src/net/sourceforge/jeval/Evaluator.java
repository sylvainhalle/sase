...
public class Evaluator {
	...
	public void parse(final String expression) throws EvaluationException {
		...
		if (!expression.equals(previousExpression)) {
			...
			operandStack = (Stack<Object>) previousOperandStack.clone();
		}
		...
		try {
			if (parse) {
				...
				previousOperandStack = (Stack<Object>) operandStack.clone();
			}
		} catch (Exception e) {
	...
	private int processOperand(final String expression, final int charCtr,
			final int operatorIndex, final Stack<Object> operandStack,
	...
	private int processOperator(final String expression,
			...
			final Stack<ExpressionOperator> operatorStack, final Stack<Object> operandStack,
	 ...
	 *            The stack of operators.
	 */
	private void processTree(final Stack<Object> operandStack, final Stack<ExpressionOperator> operatorStack) {
	...
	private String getResult(final Stack<ExpressionOperator> operatorStack,
			final Stack<Object> operandStack, final boolean wrapStringFunctionResults)
			throws EvaluationException {
	...
	protected String processNestedFunctions(final String arguments)
		...
		if (arguments.length() > 0) {
			...
			List<String> evalautedArgumentList = new ArrayList<String>();
			...
			Iterator<String> evaluatedArgumentIterator = evalautedArgumentList
					.iterator();
